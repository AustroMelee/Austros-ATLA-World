...
| **`components/MatrixRain/MatrixRain.tsx`** | **UPDATED (2025):** Canvas-based Matrix digital rain with static character grid, brightness waves, and random glyph cycling. Streams never collide and characters flip or rotate for movie accuracy. Uses `requestAnimationFrame` for smooth 60fps animation with adaptive frame skipping. |
