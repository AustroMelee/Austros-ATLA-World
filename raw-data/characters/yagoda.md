---
type: character
---

## UI - CARD VIEW

```md
- Name: Yagoda
- Nation: Northern Water Tribe
- Badge: Healer
- Short Description: Yagoda is a skilled healer and respected elder of the Northern Water Tribe, known for teaching Katara and her compassion for her people.
```

## UI - EXPANDED VIEW

```md
### 📖 Overview

Yagoda is a master healer and waterbending instructor from the Northern Water Tribe. She taught generations of young women the art of healing and was a close friend of Kanna.

### 🧩 Role in the Story

Yagoda was a female waterbender from the Northern Water Tribe, specializing in the art of healing. She instructed young women of Agna Qel'a on how to use their waterbending abilities for healing. Yagoda was a kind and laid-back teacher, respected for her skill and wisdom. She was a close friend of Kanna, Katara and Sokka's grandmother, and recognized Katara's betrothal necklace as Pakku's work. Yagoda attempted to heal Princess Yue as a newborn, but was unable to help, leading Chief Arnook to seek aid from the Moon Spirit.

### 🤝 Relationships

- Katara: Student
- Kanna: Friend
- Northern Water Tribe: Community

### 🌟 Notable Traits

- Kind
- Patient
- Laid-back
- Traditional
- Supportive
- Calm

### 💬 Quotes

> "Are you here for the healing lesson?"
> — Yagoda

### ✨ Narrative Highlights

- Master healer and instructor in the Northern Water Tribe.
- Attempted to heal Princess Yue as a newborn.
- Recognized Katara's betrothal necklace as Pakku's work.
```

```json
{
  "id": "yagoda",
  "slug": "yagoda"
}
```

```json
{
  "id": "yagoda",
  "slug": "yagoda",
  "identity": {
    "name": "Yagoda",
    "fullName": "Yagoda",
    "aliases": [],
    "pronouns": "she/her",
    "nation": "Northern Water Tribe",
    "ethnicity": "Water Tribe",
    "gender": "Female",
    "age": 80,
    "ageRange": "elder",
    "isBender": true,
    "bendingElement": "water",
    "hairColor": "Gray (brown in youth)",
    "eyeColor": null,
    "skinColor": null,
    "loveInterest": null,
    "image": null,
    "role": "NWT Healer",
    "titles": []
  },
  "abilities": {
    "bending": ["Waterbending (Healing)"],
    "weapons": [],
    "fightingStyles": ["Healing"],
    "notableFeats": [
      "Master healer and instructor in the Northern Water Tribe.",
      "Attempted to heal Princess Yue as a newborn.",
      "Recognized Katara's betrothal necklace as Pakku's work."
    ]
  },
  "affiliations": {
    "primary": "Northern Water Tribe",
    "secondary": [],
    "allies": ["Katara", "Kanna", "Northern Water Tribe"],
    "enemies": ["Fire Nation"]
  },
  "narrative": {
    "firstAppearance": "The Waterbending Master",
    "lastAppearance": "The Siege of the North, Part 2 (flashback only)",
    "voicedBy": "Lucille Bliss",
    "era": "Book One: Water",
    "seasons": [1],
    "episodes": [18, 20],
    "notableQuotes": [
      "Are you here for the healing lesson?"
    ]
  },
  "metrics": {
    "archetype": "Healer",
    "moralAlignment": "Lawful Good",
    "intelligence": 6,
    "strength": 2,
    "charisma": 5,
    "leadership": 5,
    "humor": 3
  },
  "roles": {
    "healer": true,
    "instructor": true
  },
  "relationships": {
    "katara": "Student",
    "kanna": "Friend",
    "northernWaterTribe": "Community"
  },
  "psyche": {
    "personality": ["Kind", "Patient", "Laid-back", "Traditional"],
    "emotionalState": ["Supportive", "Calm"],
    "motivation": "To teach and heal the next generation of waterbenders."
  },
  "physical": {
    "build": "Elderly",
    "height": null,
    "distinguishingFeatures": ["Gray hair"]
  },
  "dialogue": [
    "Are you here for the healing lesson?"
  ],
  "tags": ["Northern Water Tribe", "Healer", "Instructor", "Book One", "The Waterbending Master"],
  "tagCategories": {
    "nationTags": ["Northern Water Tribe", "Water Tribe"],
    "roleTags": ["Healer", "Instructor"],
    "episodeTags": ["Book One", "The Waterbending Master"]
  },
  "system": {
    "schemaVersion": "3.0",
    "createdBy": "AI Assistant",
    "createdAt": "2024-06-09T00:00:00Z",
    "flags": []
  }
}
```
